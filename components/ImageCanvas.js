...
export const ImageCanvas = ({
  ...
  const handleClick = (e) => {
    const data = extractPixelData(e);
    // Even if clicking outside (returning null), we can trigger the handler to potentially unlock
    if (onPixelClick) {
        onPixelClick(data);
    }
  };
  ...
  return html`
    ...
    >
      ${!originalImage && html`<p className="text-slate-400">Loading...</p>`}
      <canvas
        ref=${canvasRef}
        className="max-w-full max-h-full object-contain cursor-crosshair shadow-md active:cursor-grabbing"
